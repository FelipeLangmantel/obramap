...
function ObraDetail({ obra }: { obra: ObraEnriched }) {
  const statusCfg = STATUS_CONFIG[obra.status] || STATUS_CONFIG.nao_iniciada;
  const previsaoFim =
    obra.data_inicio
      ? format(addDays(new Date(obra.data_inicio), obra.prazo_dias + obra.aditivo_prazo_dias), "dd/MM/yyyy")
  ...
  const docFields: { key: keyof DocumentosObra; label: string }[] = [
    { key: "ata", label: "Ata" },
    { key: "ois", label: "OIS" },
    { key: "art", label: "ART" },
    { key: "cno", label: "CNO" },
  ...
  return (
    <div className="space-y-5">
      <SheetHeader>
        <div className="flex items-center gap-2">
          <span className={`h-3 w-3 rounded-full ${HEALTH_COLORS[obra.health]}`} />
          <SheetTitle className="text-lg">{obra.nome}</SheetTitle>
        </div>
        <div className="flex items-center gap-2 mt-1">
          <Badge className={statusCfg.className} variant="secondary">{statusCfg.label}</Badge>
          {obra.parceria_scp && <Badge variant="outline">SCP: {obra.parceria_scp}</Badge>}
        </div>
      </SheetHeader>
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <InfoItem label="Empresa" value={obra.empresa || "—"} />
        <InfoItem label="Contrato" value={obra.num_contrato || "—"} />
        <InfoItem label="Valor Contrato" value={BRL.format(obra.valor_contrato)} />
        <InfoItem label="Andamento" value={`${obra.percentual_andamento}%`} />
        <InfoItem
          label="Início"
          value={obra.data_inicio ? format(new Date(obra.data_inicio), "dd/MM/yyyy") : "—"}
        />
        <InfoItem label="Previsão Fim" value={previsaoFim} />
        <InfoItem label="Prazo" value={`${obra.prazo_dias} dias`} />
        <InfoItem label="Aditivo Prazo" value={obra.aditivo_prazo_dias > 0 ? `+${obra.aditivo_prazo_dias} dias` : "—"} />
        <InfoItem label="Período Medição" value={obra.periodo_medicao || "—"} />
        <InfoItem label="Prazo Pagamento" value={obra.prazo_pagamento || "—"} />
      </div>
      ...
      <div>
        <h4 className="font-semibold text-sm text-foreground mb-2 flex items-center gap-1.5">
          ...
          Documentos ({obra.docsCount}/{obra.docsTotal})
        </h4>
        <div className="grid grid-cols-2 gap-1.5">
          {docFields.map((f) => {
            const ok = obra.docs ? obra.docs[f.key] === true : false;
            return (
      ...
      <div>
        <h4 className="font-semibold text-sm text-foreground mb-2 flex items-center gap-1.5">
          ...
          Medições ({obra.allMedicoes.length})
        </h4>
        {obra.allMedicoes.length === 0 ? (
          <p className="text-xs text-muted-foreground">Nenhuma medição registrada.</p>
        ) : (
          <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
            {obra.allMedicoes.map((m) => (
              <div key={m.id} className="flex items-center justify-between text-xs bg-muted/30 rounded-md px-3 py-2">
                <div className="flex items-center gap-2">
                  <MedicaoStatusBadge status={m.status_medicao} />
                  <span className="font-medium">{m.num_medicao || "—"}</span>
                  <span className="text-muted-foreground">{m.mes_referencia}/{m.ano_referencia}</span>
                </div>
                <span className="font-mono font-medium">{BRL.format(m.valor_medicao)}</span>
              </div>
            ))}
          </div>
        )}
...
function MedicaoStatusBadge({ status }: { status: string }) {
  const cfg: Record<string, { label: string; className: string }> = {
    aprovada: { label: "Aprovada", className: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300" },
  ...
  return <Badge className={`text-[10px] ${c.className}`} variant="secondary">{c.label}</Badge>;
}
