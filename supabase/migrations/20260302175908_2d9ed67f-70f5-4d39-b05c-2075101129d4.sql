...
BEGIN
  ...
  all_executed AS (
    -- Productions table: EXCLUDE banco inicial (is_initial_database=true)
    -- because weekly_productions is the authoritative source for banco inicial
    SELECT 
      p.macro_id,
      p.scope_id,
      p.macro_name,
    ...
    -- Weekly productions table: ALL entries (including banco inicial)
    SELECT 
      wp.macro_id,
      wp.scope_id,
      wp.macro_name,
