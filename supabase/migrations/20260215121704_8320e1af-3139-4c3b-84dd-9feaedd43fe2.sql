...
BEGIN
  ...
  WITH project_houses AS (
    ...
    FROM houses
    WHERE houses.project_id = p_project_id
  ),
  -- Combine productions and weekly_productions, unnest house_ids to count distinct houses per service
  all_executed AS (
    SELECT 
      p.macro_id,
      p.scope_id,
      MAX(p.macro_name) as macro_name,
      MAX(p.scope_name) as scope_name,
      unnest(p.house_ids) as house_number
    FROM productions p
    ...
    SELECT 
      wp.macro_id,
      wp.scope_id,
      MAX(wp.macro_name) OVER (PARTITION BY wp.macro_id, wp.scope_id),
      MAX(wp.scope_name) OVER (PARTITION BY wp.macro_id, wp.scope_id),
      unnest(wp.house_ids) as house_number
  ...
  SELECT 
    COALESCE(e.macro_id, pcs.macro_id)::text as macro_id,
    COALESCE(e.scope_id, pcs.scope_id)::text as scope_id,
    COALESCE(e.macro_name, pcs.macro_name)::text as macro_name,
    COALESCE(e.scope_name, pcs.scope_name)::text as scope_name,
    ph.total as total_houses,
    COALESCE(e.executed, 0)::bigint as executed_houses,
    GREATEST(0, ph.total - COALESCE(e.executed, 0))::bigint as available_houses,
  ...
  CROSS JOIN (
    SELECT DISTINCT pcs_inner.macro_id, pcs_inner.scope_id, pcs_inner.macro_name, pcs_inner.scope_name
