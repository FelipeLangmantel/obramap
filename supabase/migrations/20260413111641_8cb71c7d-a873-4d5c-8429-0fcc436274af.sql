...
-- Fix fn_gerar_alertas_operacionais: use data_inicio + prazo_dias instead of non-existent data_fim_prevista
-- Also add alert for overdue measurements (previsão vencida)
...
DECLARE
  v_data_fim date;
BEGIN
  -- 1. Prazo da obra vencido (data_inicio + prazo_dias < today)
  ...
  WHERE status = 'em_andamento'
    AND data_inicio IS NOT NULL AND prazo_dias IS NOT NULL
  ...
  SELECT company_id, id, 'prazo_obra_avencer',
    ...
    format('Obra "%s" vence em %s (%s dias).', nome,
      to_char((data_inicio + prazo_dias),'DD/MM/YYYY'), ((data_inicio + prazo_dias) - CURRENT_DATE)),
    'holding'
  FROM obras_portfolio
  WHERE status = 'em_andamento'
    AND data_inicio IS NOT NULL AND prazo_dias IS NOT NULL
  ...
  -- 3. Medição com previsão de envio vencida (prevista + data_previsao < today + sem envio)
  ...
  SELECT op.company_id, m.obra_id, m.id, 'medicao_previsao_vencida',
    ...
    format('Medição %s da obra "%s" prevista para %s ainda não foi enviada (%s dias de atraso).',
  ...
  WHERE m.status_medicao IN ('prevista','nao_iniciada')
    AND m.data_previsao_medicao IS NOT NULL
    AND m.data_previsao_medicao < CURRENT_DATE
    AND m.data_envio IS NULL
    AND NOT EXISTS (SELECT 1 FROM system_notifications sn
      WHERE sn.medicao_id = m.id AND sn.tipo = 'medicao_previsao_vencida' AND sn.resolvida = false);

  -- 4. Obra sem medição dentro do período cadastrado
  INSERT INTO system_notifications (company_id, obra_id, tipo, titulo, mensagem, modulo)
  ...
  SELECT op.company_id, m.obra_id, m.id, 'desvio_financeiro_relevante',
    ...
    format('Medição %s: previsto %s, acatado %s (desvio %.1f%%).',
      COALESCE(m.num_medicao,'—'),
      to_char(m.valor_previsto_medicao,'FM"R$"999G999G990D00'),
      to_char(m.valor_acatado,'FM"R$"999G999G990D00'),
  ...
  -- Auto-resolver alertas de medição vencida quando a medição já foi enviada
  ...
  -- Auto-resolver alertas de prazo vencido quando a obra não está mais em andamento
  UPDATE system_notifications SET resolvida = true, resolvida_em = now()
...
EXCEPTION WHEN OTHERS THEN
  RAISE LOG 'Error in fn_gerar_alertas_operacionais: % %', SQLERRM, SQLSTATE;
  RAISE;
END;
$$;
