...
BEGIN
  ...
  -- 3. Medição com previsão de envio vencida
  ...
  -- 4. Obra sem medição dentro do período cadastrado (use data_previsao_medicao instead of created_at)
  INSERT INTO system_notifications (company_id, obra_id, tipo, titulo, mensagem, modulo)
  ...
  WHERE op.status = 'em_andamento' AND op.periodo_medicao IS NOT NULL
    AND NOT EXISTS (
      SELECT 1 FROM medicoes_ple m WHERE m.obra_id = op.id
        AND COALESCE(m.data_envio, m.data_previsao_medicao) > (CURRENT_DATE - (
  ...
  -- Auto-resolver alertas de prazo quando a obra não está mais em andamento
  UPDATE system_notifications SET resolvida = true, resolvida_em = now()
