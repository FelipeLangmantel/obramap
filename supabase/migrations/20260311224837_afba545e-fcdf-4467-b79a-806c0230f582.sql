...
-- ============================================================
-- Central atomic function for all structural mutations
...
DECLARE
  ...
  v_company_id uuid;
BEGIN
  -- 1. Read current state (single source of truth)
  ...
  -- 4. CASCADE DELETIONS for removed scopes (all dependent tables, atomic)
  ...
  -- 5. CASCADE DELETIONS for removed macros (broader cleanup)
  ...
  -- 6. PROPAGATE NAME/COLOR CHANGES for surviving scopes
  ...
  WHERE cs.project_id = p_project_id
    ...
    AND (cs.scope_name IS DISTINCT FROM scope->>'name' OR cs.macro_name IS DISTINCT FROM macro->>'name');
  ...
  WHERE sp.project_id = p_project_id
    AND sp.scope_id = scope->>'id'
    AND (sp.scope_name IS DISTINCT FROM scope->>'name' OR sp.macro_name IS DISTINCT FROM macro->>'name');
  ...
  -- 7. PROPAGATE MACRO-LEVEL name/color changes
  UPDATE weekly_productions wp
  ...
  -- 8. Apply the new template to the project
  UPDATE projects
  SET macros_template = p_new_template
  WHERE id = p_project_id;

  -- 9. Sync contract services (remove stale, add new if contract exists)
  PERFORM sync_contract_services(p_project_id, v_company_id);

  -- 10. Return delta summary
