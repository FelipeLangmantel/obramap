...
BEGIN
  ...
  SELECT company_id, id, 'prazo_obra_avencer',
    ...
    format('Obra "%s" vence em %s (%s dias).', nome,
      to_char(data_fim_prevista,'DD/MM/YYYY'), (data_fim_prevista - CURRENT_DATE))
  FROM obras_portfolio
  ...
  -- 3. Obra sem medição dentro do período cadastrado
  INSERT INTO system_notifications (company_id, obra_id, tipo, titulo, mensagem)
  ...
  -- 4. Medição prevista sem data de previsão
