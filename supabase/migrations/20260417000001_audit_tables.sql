-- ══════════════════════════════════════════════════════════
-- Tabelas de auditoria para exclusões e correções de produção
-- ══════════════════════════════════════════════════════════

-- 1. Log de exclusões de produção (somente admin)
CREATE TABLE IF NOT EXISTS public.production_deletion_log (
...
CREATE INDEX IF NOT EXISTS idx_deletion_log_project
  ON public.production_deletion_log(project_id, created_at DESC);


-- 2. Correções de itens do diário pelo coordenador
CREATE TABLE IF NOT EXISTS public.diary_item_corrections (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id UUID NOT NULL REFERENCES public.companies(id) ON DELETE CASCADE,
  project_id UUID NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_corrections_entry
  ON public.diary_item_corrections(diary_entry_id);

CREATE INDEX IF NOT EXISTS idx_corrections_project
  ON public.diary_item_corrections(project_id, created_at DESC);
