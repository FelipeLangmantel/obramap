CREATE OR REPLACE FUNCTION public.get_service_execution_bank(p_project_id uuid)
 RETURNS TABLE(macro_id text, scope_id text, macro_name text, scope_name text, total_houses bigint, executed_houses bigint, available_houses bigint, completion_percent numeric, status text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  all_executed AS (
    -- Productions table: EXCLUDE banco inicial e soft-deleted
    SELECT 
      p.macro_id,
      p.scope_id,
      p.macro_name,
    ...
    WHERE p.project_id = p_project_id
      ...
      AND array_length(p.house_ids, 1) > 0
      AND COALESCE(p.is_initial_database, false) = false
      AND p.deleted_at IS NULL
    
    UNION ALL
    
    -- Weekly productions table: ALL entries (incluindo banco inicial), exceto soft-deleted
    SELECT 
      wp.macro_id,
      wp.scope_id,
      wp.macro_name,
    ...
    WHERE wp.project_id = p_project_id
      ...
      AND array_length(wp.house_ids, 1) > 0
